/**
 * read .global or global.json file and
...
const { lineToObject, formatLine } = require("./utils");
const fs = require("fs");
 ...
 * @returns {Object}  type:{ MODULE, CONFIG }
 */
module.exports = function(path) {
  //if file is global.json
  if (path.split(".").reverse()[0] === "json") {
    
    let obj = require(path);
    if (checkJson(obj)) return obj;
    else throw new Error("global.json file is not valid.");
  }
  ...
  lines.forEach(line => {
    ...
    if (argument === "DEFINE") {
      ...
      let {key, value} = lineToObject(parameters.join(''));
