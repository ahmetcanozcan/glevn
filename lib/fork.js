 ...
 * @returns {void}
 */
function forkChildProcess(execFile, options, hasGlevmon, glevnfile) {
  ...
  let invoke = () => {
    ...
    if (glevnfile) {
      console.debug('Set glevnfile event');
      forked.on('message', ({
        head,
        body
      }) => {
        console.debug('message from', head.from);
        ...
        if (head.from === 'app') {
          const msg = new Message()
            .from('glevn')
            .add('MODULE', glevnfile.MODULE)
            .add('CONFIG', glevnfile.CONFIG)
          console.debug('send message to app', msg);
          forked.send(msg);
        }
      })
    }
